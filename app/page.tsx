...
interface CarData {
  ...
  hasAWD?: boolean; drivetrains?: string[]; avgConsumption?: number | null; fuelTankLiters?: number;
...
function getRecommendedTransmission(ans: Answers): string {
 ...
 const high = isHighBudget(budget);
  ...
  if (env === "city") return "prefer_auto";
...
// ════════════════════════════════════════════════════════════
// SCORING ALGORITHM — updated for new quiz answers
// ════════════════════════════════════════════════════════════
...
const LIFTBACK_STYLES = ["liftback", "sportback", "fastback"];
  ...
  console.log("VARIANT CHECK:", c.make, c.model, "variants:", c.bodyVariants?.length, "body:", c.body);
  ...
  const wantsLow = bt === "small_nimble"
    ...
    console.log("VARIANT LOGIC:", "space:", space, "mission:", mission, "needsCargo:", needsCargo, "wantsLow:", wantsLow);
  ...
  for (const v of variants) {
    ...
    console.log("  VARIANT:", style, "isEstate:", isEstate, "fires:", isEstate && needsCargo && !wantsLow);
...
function hardFilter(c: CarData, a: Answers): boolean {
 ...
 if ((c.budgetMin || 0) > budget * 1.2) return false;
  ...
  if (env === "rural" && gc < 130 && !["suv", "pickup"].includes(body)) return false;
  ...
  if (bt === "small_nimble" && ["suv", "pickup", "van", "estate"].includes(body)) return false;
...
function scorePractical(c: CarData, a: Answers): number {
 ...
 // ── BOOT SPACE ──
  ...
  else if (space === "moderate" && effBoot < 250) s -= 4;
 ...
 if (bt === "raised" && body === "suv") s += 4;
  ...
  else if (bt === "normal" && ["pickup", "van"].includes(body)) s -= 4;
...
function scoreFinancial(c: CarData, a: Answers): number {
  ...
  else if (midPrice < budget * 0.3) s += imageMission ? -5 : 2; // Image buyers: a bargain-bin car defeats the purpose
...
function calcResults(DB: CarData[], a: Answers): ScoredCar[] {
  ...
  const sorted = normalized.filter((s) => s.score > 0).sort((a, b) => b.score - a.score).slice(0, 20);
...
export default function HomePage() {
  ...
  const startLoading = useCallback(() => {
    ...
    setTimeout(() => {
      try {
        const scored = calcResults(DB, ans);
        setResults(scored);
    ...
    <div className="w">
      ...
      {phase === "results" && (<>
        ...
        {results.map(({ car, score, isBest }, i) => {
          const c = norm(car); const fd = faultDB[c.id]; const isOpen = openMore[c.id];
          ...
          return (
            <div key={c.id} className={`card${isBest ? " best" : ""}`} style={{ animationDelay: `${i * 0.05}s` }}>
              ...
              <div className="cgrid" style={{ marginTop: 0 }}>
                {dispCons && <div className="cgrid-item"><div className="cgrid-label">Consumption</div><div className="cgrid-val">{dispCons} l/100{isUnpluggedPhev && <span style={{ fontSize: "0.6rem", color: "#ff9944", marginLeft: 4 }}>{"⚠️"} if charged</span>}</div></div>}
