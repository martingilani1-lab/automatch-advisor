// Standalone scoring-engine snapshot for offline profile/regression audits
// (e.g. before/after diffs when changing scoring calibration) — reads raw,
// pre-normalization scores for every car, which /api/recommend never exposes
// (it only returns the top-10 post-calcResults). Not imported by the app.
//
// This is a manually-synced MIRROR of app/api/recommend/route.ts's fetch +
// scoring pipeline, not a shared import — if that file's scoring logic
// changes, re-sync this file by hand (diff against it) or the audit will
// silently test stale logic. Same duplication tradeoff CLAUDE.md already
// accepts for the small per-fuel helpers, just larger in scope here.
//
// Usage: npx tsx scripts/score-audit.ts <profiles.json> <output.json>
// profiles.json: [{ "name": string, "answers": Answers }, ...]
// output.json: { [profileName]: { id, name, raw, max, normalized }[] }
//
// IMPORTANT — deep-clone per profile: fetchCarData() below is called ONCE
// and the same car objects are scored against every profile in profiles.json.
// norm() mutates a car in place and short-circuits via its `_n` flag on
// repeat calls, and applyBestVariant() also mutates body/boot in place. Score
// profile N+1 against a structuredClone of the fetched cars, never the same
// objects profile N already scored — see fullResults() below — or later
// profiles silently score against leftover state from earlier ones. The app
// itself is unaffected (fetchCarData() runs fresh per HTTP request), but any
// harness that loops multiple profiles over one fetch must do this.
...
interface CarData {
  ...
  reliability?: any; reliabilityByFuel?: Record<string, string> | null; safety?: any; equipment?: any[];
...
// ════════════════════════════════════════════════════════════
// AUDIT HARNESS (not part of the app — analysis only)
// ════════════════════════════════════════════════════════════

// Raw (pre-normalization) + normalized score for every car, given one fetch.
// Takes its own DB slice so callers can pass a fresh structuredClone per
// profile — see the deep-clone note in the file header.
function fullResults(DB: CarData[], a: Answers) {
  const raw = DB.map((car) => ({ id: car.id, name: `${car.make} ${car.model}`, score: scoreCar(car, a) }));
  const valid = raw.filter((s) => s.score > 0);
  const max = valid.length ? Math.max(...valid.map((s) => s.score)) : 0;
  return raw.map((s) => ({
    id: s.id, name: s.name, raw: s.score, max,
    normalized: s.score === 0 ? 0 : Math.max(10, Math.min(99, Math.round((s.score / max) * 99))),
  }));
}

async function main() {
  const profilesPath = process.argv[2] || path.join(__dirname, "score-audit-profiles.json");
  const outPath = process.argv[3] || path.join(__dirname, "score-audit-output.json");
  const profiles = JSON.parse(fs.readFileSync(profilesPath, "utf8"));
  const cars = await fetchCarData();
  const out: Record<string, any[]> = {};
  for (const p of profiles) {
    out[p.name] = fullResults(structuredClone(cars), p.answers);
  }
  fs.writeFileSync(outPath, JSON.stringify(out, null, 2));
  console.error(`Scored ${profiles.length} profiles against ${cars.length} cars -> ${outPath}`);
}
main();
