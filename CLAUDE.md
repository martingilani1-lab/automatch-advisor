...
- `recommend/route.ts` — a **separate, self-contained** scoring endpoint (Slovak comments/error messages) that does its own budget/boot/seats/fuel scoring server-side. It is **not called by the frontend** — the live recommendation logic is the client-side engine in `page.tsx` (see below). Don't assume this endpoint is the source of truth for scoring; treat it as a parallel/legacy implementation unless told otherwise.
...
**Frontend** (`app/page.tsx`, single client component, ~1500 lines): everything — quiz, scoring engine, and results UI — lives in this one file.
- `QUESTIONS` defines the 16-question quiz (`QuizQuestion[]`), driving a `phase` state machine: `hero → quiz → loading → results`.
- On mount, all cars are fetched once from `/api/cars` into a `DB` state array.
- Quiz answers accumulate into an `Answers` object keyed by question id.
- Once the quiz completes, `calcResults(DB, ans)` runs the actual recommendation algorithm entirely client-side:
  - `hardFilter` — excludes vehicles that fail hard constraints (seats, budget floor, etc.)
  - `scorePractical`, `scoreFinancial`, `scorePreference`, `scoreSafety` — dimension scores
  - `scoreCar` — combines the dimension scores (plus fuel/transmission recommendation heuristics from `getRecommendedFuel`/`getRecommendedTransmission`) into a final ranked score
  - Results are `ScoredCar[]`, sorted and flagged with `isBest`
- Expanding a result card lazily calls `/api/detail` per vehicle and caches the response in `faultDB`.

When changing recommendation behavior, the client-side pipeline in `page.tsx` (`hardFilter` → `score*` functions → `scoreCar` → `calcResults`) is the one actually driving the UI — `app/api/recommend` is not wired up to it.
...
1. Scoring runs **client-side** in `app/page.tsx` (`calcResults`, `app/page.tsx:1025`) and returns the **top 20** (`.slice(0, 20)`). This is deliberate during testing. Do not move scoring to `app/api/recommend`, and do not change the `slice(0, 20)` cutoff.
2. Progressive disclosure (car detail expansion) uses expandable panels, not a Shadcn `Dialog`. This is deliberate. Do not convert expandable panels to a dialog/modal component.
3. Scoring uses base values in the 6–8 range and `Math.min(35, s)` caps on each dimension score (`scorePractical`, `scoreFinancial`, `scorePreference`, `scoreSafety`), and `scoreCar` clamps the combined result at 200 (`Math.min(200, ...)`, `app/page.tsx:1014`). These weights/caps are deliberately calibrated — never change these numbers.
...
- Pickups have `NULL boot_capacity_liters` in the DB by design; scoring accounts for this via `effBoot` (`app/page.tsx:628`, e.g. `body === "pickup" ? Math.max(boot, 1000) : boot`) rather than expecting real boot data for that body type.
